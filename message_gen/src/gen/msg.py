...
def summarize(diffs: list[Diff], original: str, v: int) -> str:
    ...
    stream = client.chat.completions.create(
        messages=messages, max_tokens=200, temperature=0.2, stream=True
    )
...
def summarize_chain(diffs: list[Diff], original: str) -> str:
    messages = [
        {"role": "system", "content": system_msg_summarize},
        {
            "role": "user",
    ...
    # print("input: ", "\n".join([str(diff) for diff in diffs]))
    stream = client.chat.completions.create(
        messages=messages, max_tokens=500, temperature=0.3, stream=True
    )
    msg = "".join([chunk.choices[0].delta.content for chunk in stream])
    print("output: ", msg)
    messages = [
        ...
        },
        {
            ...
            + msg
            + "base the commit message on the diff and the summary",
        },
    ]
    # print("input: ", "\n".join([str(diff) for diff in diffs]))
    stream = client.chat.completions.create(
